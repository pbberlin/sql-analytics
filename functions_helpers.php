...
function get_puppet_hosts_by_role( $_role = "mysql" ) { 
	...
	$_cn2 = mysql_connect( "mysql-db1" , "idealo" , "s3cr3t" , true , MYSQL_CLIENT_COMPRESS );
...
function array_to_table($a, $_arr_options=array() ){
	...
	$_heading = '';
	$_cutoff = 0;
	$_cols_with_popup = array();			# make val_col to appear as popup over key_col, remove val_col
	extract( expand_options_array($_arr_options) );
	

	$_cols_mouseover_src = array_flip($_cols_with_popup);
   ...
   $_class_not_first = "";
	...
	foreach( $a[0] as $_col_name => $_val_unused ){
		...
		$_col_name_by_idx[$_col_counter] = $_col_name;
		$_col_counter++;

		if( $_cols_mouseover_src[$_col_name] ) {  continue; }

		$_hint_mouse_over = "";
		$_name_col_popup = $_cols_with_popup[$_col_name];
		if( $_name_col_popup ) {  
			$_hint_mouse_over = " <span style='font-size:9px'>...(mouseover)</span>";
		}

		$t.=	"<th $_class_not_first >{$_col_name}{$_hint_mouse_over}</th>";		
	   ...
	   $t.="<tr>";
		...
		foreach( $row as $_col_name => $_val ){
			...
			if( $_name_col_popup ) {  
				...
				$_tmp_val = $row[$_name_col_popup];
				$_tmp_val = replace_non_ascii($_tmp_val, " " );
				$_attr_title = " title='$_tmp_val' ";

				/*
				$_dbg = "";
				$_dbg .= "--" . $_lp_col_name;
				$_dbg .= "--" . $_name_col_popup;
				$_dbg .= "--" . $_idx_col_popup;
				$_dbg .= "--" . $_tmp_val;
				*/
			...
			$t.=	"\n\t<td $_class_not_first  $_attr_title >{$_val}</td>";		
			$_class_not_first = " class='class_not_first' ";
		}
	   $t.="</tr>\n";
...
function render_primary_navigation(){
	...
	$_php_my_admin_token = get_param("token");
	$_php_my_admin_server_id = get_param("server");
	$_php_my_admin = "?&server={$_php_my_admin_server_id}&token={$_php_my_admin_token}";

	$_arr_opt2["additional_attributes"] = "";
	$_str_ret .= render_inline_block( "<A style='$_style_text2'  target='_top' HREF='/crawler/{$_php_my_admin}'        >PHP MyAdmin nach Funktion</A>",false, $_arr_opt2 ) ;
	$_str_ret .= render_inline_block( "<A style='$_style_text2'  target='_top' HREF='/crawler-admin/{$_php_my_admin}'  >PHP MyAdmin nach Maschine</A>",false, $_arr_opt2 ) ;
